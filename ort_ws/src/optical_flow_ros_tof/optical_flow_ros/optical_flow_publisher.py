...
class OpticalFlowPublisher(Node):
    def __init__(self, node_name="optical_flow"):
        ...
        self._tof_sub = self.create_subscription(
            Float32, "/distance_sensor/optical_flow", self._tofCB, qos_profile=qos_profile_sensor_data
        )
    ...
    def publish_diagnostics(self, status):
        if self._diagnostics_pub is not None and self._diagnostics_pub.is_activated:
            ...
            if status == DiagnosticStatus.OK:
                ...
                status_msg.message = (
                    f"Sensor error: {self._consecutive_failures} consecutive failures"
                )
    ...
    def _tofCB(self, msg: Float32):
        self._pos_z = msg.data
