...
class StreamServer:
    ...
    ### Configure for dual Camera feed ---
    def configure_dual(
        self, main_width, main_height, display_width, display_height, video_format="RGB888"
    ):
        if not self.cam is None:
            config = self.cam.create_preview_configuration(
                main={
                    ...
                    "format": video_format,
                },  # can use YUV420 and just extract Y for greyscale?
                lores={"size": (display_width, display_height), "format": video_format},
                display="lores",
            )
            self.cam.configure(self.cam.create_video_configuration(config))
            self.output.write("INFO", f"Applied config to camera {self.model}", True)
    ...
    def start(self, ip, port, multicast=True):
        if not self.cam is None:
            ...
            self.output.write(
                "INFO",
                f'Started UDP stream "{self.name}" from camera {self.model} to {ip}:{port}',
                True,
            )
